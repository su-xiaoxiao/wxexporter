#!/usr/bin/env python3
"""Scrapling-based WeChat article fetcher.

Called as a subprocess by the TS core (ScraplingRunner). Returns JSON on stdout:
    {"title": str, "cover_url": str, "markdown": str}

Adapted from maolai7/agent-skills scrapling-article-fetch. One change:
    max_chars defaults to None (no truncation) — the parent decides whether to
    truncate. Previously defaulted to 30000, silently cutting long articles
    (eng review P3 critical gap).
"""
import json
import re
import sys
...
def normalize_html_for_images(html: str) -> str:
    # WeChat lazy-loads images via data-src; hoist to src so html2text keeps them.
    return re.sub(
        r'<img([^>]*?)\sdata-src="([^"]+)"([^>]*?)>',
...
def extract_markdown(url: str, max_chars=None) -> str:
    return extract_article(url, max_chars)["markdown"]


def extract_article(url: str, max_chars=None) -> dict:
    ...
    # max_chars=None means no truncation (default). Caller may pass an int to truncate.
...
def main() -> int:
    ...
    url = args[0]
    # max_chars is optional: pass an int to truncate, omit to keep the full article.
